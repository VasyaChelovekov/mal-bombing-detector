...
class MALBombingAnalyzer:
    ...
    def print_top_bombed(self, n: int = 20):
        ...
        for m in top_metrics:
            ...
            print(f"     Bombing Score: {m.bombing_score:.2f} ({m.suspicion_level.value.upper()})")
            print(f"     Score '1': {m.ones_percentage:.2f}% | "
                  f"Score '10': {m.tens_percentage:.2f}%")
            print(f"     Z-score: {m.ones_zscore:.3f} | "
...
def main():
    ...
    parser.add_argument(
        '--no-cache',
        action='store_true',
        help='Do not use cache (reload all data)'
    )
    parser.add_argument(
        '-v', '--verbose',
        action='store_true',
