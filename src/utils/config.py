...
class AnalysisConfig:
    ...
    suspicion_thresholds: SuspicionThresholdsConfig = field(
        default_factory=SuspicionThresholdsConfig
    )
    statistical_thresholds: StatisticalThresholdsConfig = field(
...
class ConfigLoader:
    ...
    def _load_analysis_config(cls, config: Config, data: Dict[str, Any]) -> None:
        ...
        config.analysis.age_old_threshold_years = age_data.get(
    ...
    def _load_visualization_config(cls, config: Config, data: Dict[str, Any]) -> None:
        ...
        config.visualization.output_directory = data.get(
...
class ConfigView:
    """Read-only proxy over Config (not a snapshot); blocks attribute reassignment."""
    ...
    def default_platform(self) -> str:
        return self._config.default_platform
...
def get_config_view() -> ConfigView:
    """Get a read-only view of the global configuration."""
    return ConfigView(get_config())
