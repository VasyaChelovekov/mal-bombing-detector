...
class ChartGenerator:
    ...
    def generate_all(
        self, results: list[AnalysisResult], prefix: str = ""
    ) -> list[Path]:
        """
    ...
    def severity_distribution(
        ...
        # Save
        filename = (
            f"{prefix}severity_distribution" if prefix else "severity_distribution"
        )
    ...
    def suspicion_scores(self, results: list[AnalysisResult], prefix: str = "") -> Path:
        """
        ...
        sorted_results = sorted(
            results, key=lambda r: r.metrics.suspicion_score, reverse=True
        )[:20]  # Top 20
    ...
    def score_distribution_comparison(
        ...
        for idx, (row_results, row_title) in enumerate(
            [(suspicious, "Most Suspicious"), (clean, "Least Suspicious")]
        ):
            for col, r in enumerate(row_results):
                ...
                percentages = [
                    v / sum(votes) * 100 if sum(votes) > 0 else 0 for v in votes
                ]
        ...
        # Save
        filename = (
            f"{prefix}distribution_comparison" if prefix else "distribution_comparison"
        )
        path = self._get_output_path(filename)
        plt.savefig(path, dpi=150, bbox_inches="tight")
        plt.close()
    ...
    def single_distribution(self, result: AnalysisResult, prefix: str = "") -> Path:
        """
        ...
        filename = (
            f"{prefix}distribution_{anime_id}" if prefix else f"distribution_{anime_id}"
        )
        path = self._get_output_path(filename)
        plt.savefig(path, dpi=150, bbox_inches="tight")
        plt.close()
    ...
    def metrics_correlation(
        ...
        sns.heatmap(
            corr, annot=True, fmt=".2f", cmap="RdYlBu_r", center=0, ax=ax, square=True
        )
