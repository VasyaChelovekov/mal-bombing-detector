...
class ExcelExporter(BaseExporter):
    ...
    def __init__(self, output_config: OutputConfig, i18n: I18nManager | None = None):
    ...
    def _create_summary_sheet(
        self, wb: Workbook, results: list[AnalysisResult]
    ) -> Worksheet:
    ...
    def _create_distribution_sheet(
        ...
        for row_idx, m in enumerate(all_metrics, 2):
            ...
            ws.cell(
                row=row_idx, column=3, value=round(m.ones_percentage, 2)
            ).border = self.THIN_BORDER
            ws.cell(
                row=row_idx, column=12, value=round(m.tens_percentage, 2)
            ).border = self.THIN_BORDER
    ...
    def _apply_severity_colors(
        self, ws: Worksheet, col_idx: int, metrics_list: list
    ) -> None:
