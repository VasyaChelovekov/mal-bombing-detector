    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'AnimeData':
        ...
        if data.get('distribution'):
            ...
            distribution = ScoreDistribution(
                percentages={int(k): v for k, v in data.get('score_distribution', {}).items()},
                vote_counts={int(k): v for k, v in data.get('vote_counts', {}).items()},
                total_votes=data.get('total_votes', 0),
            )
...
class ReviewBombingMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            # Legacy fields
            'extreme_rating_ratio': self.extreme_rating_ratio,
            'low_score_anomaly': self.low_score_anomaly,
            'distribution_deviation': self.distribution_deviation,
            'rating_mismatch': self.rating_mismatch,
